...
seebeck_uVK(eta::Real) = KPM.KB_OVER_E_UV_PER_K * seebeck_dimensionless(eta)
...
function verify()
    ...
    peak_eta, peak_value = golden_maximum(eta -> abs(seebeck_dimensionless(eta)), 1.0, 3.0)
...
function read_cache(path, params)
    ...
    try
        ...
        if hasproperty(payload, :params) &&
           payload.params == params &&
           hasproperty(payload, :mu) &&
           size(payload.mu) == (NC, NC) &&
           hasproperty(payload, :mu_periodic) &&
           size(payload.mu_periodic) == (NC, NC)
            return payload
        end
        println("Cache exists but parameters/schema do not match; recomputing moments.")
    catch err
        @warn "Could not read moment cache; recomputing." path exception=(
            err,
            catch_backtrace(),
        )
    end
...
relative_difference(x, y) =
    (!isfinite(x) || !isfinite(y)) ? Inf : abs(x - y) / max(abs(x), abs(y), eps(Float64))
...
function main()
    ...
    @printf(
        "Nx=Ny=%d (NH=%d), t=%.1f, NC=%d, NR=%d, arr_size=%d\n",
        Nx,
        NH,
        t,
        NC,
    ...
    @printf(
        "fixed a=%.1f, b=%.1f, pi*a/NC=%.6f; kBT=%s\n",
        a,
        b,
        pi * a / NC,
        string(kBT_values)
    ...
    @printf(
        "eta grid=[%.1f, %.1f], %d points; area=%.6f; g_J=%.1f; seed=%d\n",
        first(eta_values),
        last(eta_values),
        length(eta_values),
        area,
    ...
    println(
        "Clean finite-size control: inversion-symmetric 4x4 twist grid (16 moment matrices).",
    )
    println(
        "The periodic twist is included; stochastic probes come from one Xoshiro(seed) stream.",
    )
    println("Dirac gate restriction: |mu| <= 0.35t at each temperature.")
    println(
        "Points outside that restriction are computed and plotted with hollow markers, but excluded from every gate.",
    )
    ...
    for (eta, expected) in verification.anchors
        ...
        @printf(
            "eta=%5.1f  quadrature=% .9f  anchor=% .9f  |delta|=%.2e\n",
            eta,
            actual,
            expected,
            abs(actual - expected)
    ...
    for (eta, actual, closed) in verification.denominator_checks
        @printf(
            "A0 eta=%4.1f  quadrature=%.12f  closed=%.12f  |delta|=%.2e\n",
            eta,
            actual,
            closed,
            abs(actual - closed)
    ...
    @printf(
        "Peak: |S|/(kB/|e|)=%.9f at |eta|=%.8f\n",
        verification.peak_value,
        verification.peak_eta
    )
    @printf(
        "Mott check at eta=10: relative difference = %.6f%%\n",
        100 * verification.mott_rel
    )
    println(
        "Deterministic reference assertions passed: anchors, A0, S(0), oddness, peak, and Mott tail.",
    )
    ...
    if payload === nothing
        ...
        for (twist_index, (theta_x, theta_y)) in enumerate(twists)
            ...
            @printf(
                "twist %2d/%d: (theta_x/pi, theta_y/pi)=(% .2f,% .2f)\n",
                twist_index,
                length(twists),
                theta_x / pi,
                theta_y / pi
    ...
    mu_periodic_projected, periodic_ph_residual = particle_hole_project(payload.mu_periodic)
    mxx = KPM.ConductivityMoments(mu_projected, a, b, NH, NR)
    mxx_periodic = KPM.ConductivityMoments(mu_periodic_projected, a, b, NH, NR)
    @printf(
        "Stored twist-averaged conductivity moment matrix: %d x %d ComplexF64\n",
        size(payload.mu)...
    )
    @printf(
        "Raw odd-(n+m) moment residual: average %.3e, periodic %.3e\n",
        raw_ph_residual,
        periodic_ph_residual
    )
    ...
    sigma_periodic =
        KPM.transport_distribution(mxx_periodic, E_grid; volume = area, g_J = g_J)
    ...
    shape_deviations =
        abs.(sigma_normalized[shape_indices] .- dirac_shape[shape_indices]) ./
        dirac_shape[shape_indices]
    sigma_shape_deviation = maximum(shape_deviations)
    sigma_evenness =
        maximum(abs.(sigma[shape_indices] .- reverse(sigma)[shape_indices])) /
    ...
    periodic_shape_deviation = maximum(
        abs.(sigma_periodic_normalized[shape_indices] .- dirac_shape[shape_indices]) ./
        dirac_shape[shape_indices],
    )
    periodic_evenness =
        maximum(
            abs.(sigma_periodic[shape_indices] .- reverse(sigma_periodic)[shape_indices]),
        ) / maximum(abs, sigma_periodic[shape_indices])
    @printf(
        "Sigma(E) shape window: %.2f <= |E| <= %.2f; E_ref=%.2f\n",
        sigma_window...,
        E_ref
    )
    @printf(
        "Periodic-only shell diagnostic: max shape deviation %.3f%%; evenness %.3e\n",
        100 * periodic_shape_deviation,
        periodic_evenness
    )
    @printf(
        "Sigma(E_ref)=%.6e; max normalized |E|-shape relative deviation = %.3f%%\n",
        sigma_ref,
    ...
    for kBT in kBT_values
        ...
        @printf(
            "kBT=%.2f: accepted %d/%d points (|eta| <= %.1f); min L0 over all points = %.6e\n",
            kBT,
            length(accepted[kBT]),
            length(eta_values),
            maximum(abs, eta_values[accepted[kBT]]),
            minimum(L0_values[kBT])
        )
    end
    println(
        "L0 > 0 assertion passed at every evaluated thermoelectric point (sigma_min=0.0).",
    )
    ...
    for kBT in kBT_values
        ...
        @printf(
            "  kBT=%.2f: S(0)=% .6f uV/K; max |S(eta)+S(-eta)|=% .6f uV/K\n",
            kBT,
            S_values[kBT][zero_index],
            oddness
        )
    end
    ...
    for eta in (1.0, 4.0)
        ...
        @printf(
            "  eta=%3.1f, kBT=%.2f: S4=% .6f, S8=% .6f uV/K, rel.dS=%.3e; L0_4=%.6e, L0_8=%.6e, rel.dL0=%.3e\n",
            eta,
            kT1,
            KPM.seebeck_uVK(r4),
            KPM.seebeck_uVK(r8),
            ...
            r4.L0,
            r8.L0,
            relative_difference(r4.L0, r8.L0)
        )
    end
    ...
    for NCt in NC_values
        if NCt == NC
            ...
            mt = KPM.ConductivityMoments(mu_projected[1:NCt, 1:NCt], a, b, NH, NR)
            r = [
                KPM.thermoelectric(
                    mt,
                    eta * kT1;
    ...
    nc_max_change = maximum(abs.(trunc_S[384][nc_indices] .- trunc_S[512][nc_indices]))

    common_indices = intersect(accepted[kT1], accepted[kT2])
    collapse_difference =
        maximum(abs.(S_values[kT1][common_indices] .- S_values[kT2][common_indices]))
    ...
    max_neg_weight =
        maximum(results[kBT][i].neg_weight for kBT in kBT_values for i in accepted[kBT])
    ...
    for kBT in kBT_values
        for (carrier, side, expected_sign) in (("holes", -1, 1), ("electrons", 1, -1))
            ...
            this_ok =
                abs(abs(peak_S) / 86.3679 - 1) < 0.10 &&
                sign(peak_S) == expected_sign &&
                1.0 <= abs(peak_eta) <= 3.0
            peak_ok &= this_ok
            push!(
                peak_lines,
                @sprintf(
                    "  kBT=%.2f %-9s peak: eta=% .2f, S=% .6f uV/K (%s)",
                    kBT,
                    carrier,
                    peak_eta,
                    peak_S,
    ...
    @printf(
        "  [%s] Sigma ~ |E| within 5%% on %.2f <= |E| <= %.2f: max deviation %.3f%%\n",
        passfail(sigma_shape_deviation < 0.05),
        sigma_window...,
        100 * sigma_shape_deviation
    )
    @printf(
        "  [%s] two-temperature collapse < %.4f uV/K on common accepted points: %.6f uV/K\n",
        passfail(collapse_difference < 0.05 * KPM.KB_OVER_E_UV_PER_K),
        0.05 * KPM.KB_OVER_E_UV_PER_K,
        collapse_difference
    )
    @printf(
        "  [%s] KPM peaks within 10%% of 86.3679 uV/K with correct sign and side\n",
        passfail(peak_ok)
    )
    foreach(println, peak_lines)
    @printf(
        "  [%s] median |S_KPM-S_ref| < 5 uV/K for 1 <= |eta| <= 6 (accepted only): %.6f uV/K\n",
        passfail(median_reference_error < 5.0),
        median_reference_error
    )
    @printf(
        "  [%s] neg_weight < 1%% at accepted points: maximum %.6f%%\n",
        passfail(max_neg_weight < 0.01),
        100 * max_neg_weight
    )
    @printf(
        "  [%s] max |S_NC384-S_NC512| < 5 uV/K on full kBT=0.04 accepted grid: %.6f uV/K\n",
        passfail(nc_max_change < 5.0),
        nc_max_change
    )
    println(
        "Load-bearing qualification: the clean absolute L0 is a kernel-regularized Drude weight, not a physical bulk DC conductivity; S is the benchmarked observable.",
    )
    println(
        "Scope: the six gates share one twist-averaged moment matrix, one probe stream, and one (kernel, mesh) setting — correlated diagnostics of a finite-kernel benchmark at fixed resolution, not six independent anchors or a regulator-independence claim.",
    )
    ...
    @printf(
        "    eta    S_ref      S(%.2f)       L0(%.2f)  %s    S(%.2f)       L0(%.2f)  %s\n",
        kT1,
        kT1,
        "gate",
        kT2,
    ...
    for (i, eta) in pairs(eta_values)
        ...
        @printf(
            "  %6.2f  %9.4f  %11.4f  %12.5e  %s  %11.4f  %12.5e  %s\n",
            eta,
            S_reference[i],
            S_values[kT1][i],
            L0_values[kT1][i],
    ...
    legend_style =
        (; legend_background_color = :transparent, legend_foreground_color = :transparent)
    ...
    p1 = Plots.plot(
        ...
        linewidth = 2,
        xlabel = "E/t",
        ylabel = "Sigma(E) / Sigma(0.2t)",
        title = @sprintf("Dirac weight: %.2f <= |E| <= %.2f", sigma_window...),
        label = "KPM, 16 clean twists",
        legend = :topleft,
        legend_style...,
    )
    Plots.plot!(
    ...
    Plots.vspan!(
        p1,
        [-sigma_window[1], sigma_window[1]];
        color = :gray,
        alpha = 0.22,
        label = "kernel-rounded exclusion",
    )
    Plots.vline!(
        p1,
        [-sigma_window[2], sigma_window[2]];
        color = :gray,
        linestyle = :dot,
        label = "Dirac-window edge",
    )
    ...
    p2 = Plots.plot(
        ...
        xlabel = "eta = mu/(kBT)",
        ylabel = "S (uV/K)",
        title = "Full finite-T Dirac curve",
        label = "analytic Dirac",
        legend = :bottomright,
        legend_style...,
    )
    for (color, kBT) in zip(colors, kBT_values)
        ...
        isempty(outside) || Plots.scatter!(
            p2,
            eta_values[outside],
            S_values[kBT][outside];
            marker = :circle,
            markersize = 4.5,
            markercolor = :white,
            markerstrokecolor = color,
            markerstrokewidth = 1.5,
            label = @sprintf("outside |mu|<=0.35, %.2f", kBT)
        )
    end

    p3 = Plots.plot(;
        xlabel = "eta = mu/(kBT)",
        ylabel = "S_KPM - S_ref (uV/K)",
        title = "Residuals and Mott reference",
        legend = :topleft,
        legend_style...,
    )
    ...
    for (color, eta) in zip(panel_colors, panel_etas)
        ...
        Plots.plot!(
            p4,
            collect(NC_values),
            [trunc_S[n][i] for n in NC_values];
            color = color,
            marker = :circle,
            linewidth = 2,
