...
function cubic_hamiltonian_and_currents(L; t = 1.0, W = 0.0, rng = Xoshiro(42))
    ...
    for z = 0:(L-1), y = 0:(L-1), x = 0:(L-1)
        ...
        for (xn, yn, zn) in
            ((mod(x + 1, L), y, z), (x, mod(y + 1, L), z), (x, y, mod(z + 1, L)))
...
function report_drift(label, values)
    ...
    @printf(
        "  %s drift = % .4e uV/K, relative to final = %.2f%%\n",
        label,
        drift,
        100 * relative
    )
    if relative > 0.05
        println(
            "  $label: drift is large (>5%); this calculation is not converged at this level.",
        )
    else
...
elapsed = @elapsed begin
    ...
    @printf(
        "D = %d, rescaling a = %.4f, b = %.4f, 1/beta = %.4f, pi*a/NC = %.4f\n",
        L^3,
        h.a,
        h.b,
        inv(beta),
    ...
    mu_values = collect(
        range(h.b - 0.80 * usable_halfwidth, h.b + 0.80 * usable_halfwidth; length = 19),
    )
    ...
    for mu_chem in mu_values
        ...
        sigma_minus, sigma_plus = KPM.transport_distribution(
            mxx,
            [mu_chem - dE, mu_chem + dE];
            volume = volume,
        )
        S_mott = NaN
        if isfinite(sigma_minus) &&
           isfinite(sigma_plus) &&
           sigma_minus > 0 &&
           sigma_plus > 0
    ...
    # ------------------------------------------------------------------------
    mu_ref = clamp(-2.5, h.b - 0.70 * usable_halfwidth, h.b + 0.70 * usable_halfwidth)
    ...
    kernel_labels =
        ["Jackson", "Lorentz(lambda=3.0)", "Lorentz(lambda=4.0)", "Lorentz(lambda=5.0)"]
    kernels = Any[
        KPM.JacksonKernel,
        KPM.LorentzKernels(3.0),
        KPM.LorentzKernels(4.0),
        KPM.LorentzKernels(5.0),
    ]
    S_kernel = Float64[]
    ...
    println(
        "  A full study must also sweep L and disorder realizations; this appendix covers NC/NR/kernel only.",
    )
end
