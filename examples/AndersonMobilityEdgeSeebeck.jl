...
function cubic_hamiltonian_and_current(L, disorder; t = 1.0)
    ...
    for iz = 0:(L-1), iy = 0:(L-1), ix = 0:(L-1)
    ...
    for iz = 0:(L-1), iy = 0:(L-1), ix = 0:(L-1)
        i = site_index(ix, iy, iz, L)
        for (jx, jy, jz) in
            ((mod(ix + 1, L), iy, iz), (ix, mod(iy + 1, L), iz), (ix, iy, mod(iz + 1, L)))
...
function parse_cli(args)
    ...
    while i <= length(args)
        ...
        elseif args[i] in ("-h", "--help")
            println(
                "usage: julia AndersonMobilityEdgeSeebeck.jl [--cache PATH] [--out PATH.png]",
            )
            exit(0)
        else
...
function read_cache(path, params)
    ...
    try
        ...
        if hasproperty(payload, :params) &&
           ...
           hasproperty(payload, :mu_avg) &&
           hasproperty(payload, :mu_realizations) &&
...
function same_result(r1, r2)
    return same_number(r1.L0, r2.L0) &&
           same_number(r1.L1, r2.L1) &&
...
function main()
    ...
    @printf(
        "L=%d (NH=%d), W=%.1f, t=%.1f, NC=%d, NR=%d, disorder pairs=%d\n",
        L,
        NH,
        W,
        t,
    ...
    @printf(
        "fixed a=%.2f, b=%.1f, pi*a/NC=%.6f; kBT=%s\n",
        a,
        b,
        pi * a / NC,
        string(kBT_values)
    ...
    @printf(
        "Ec=%.3f (collapse-localized effective transport edge; historical critical estimate %.1f), x_ref=%.1f\n",
        Ec,
        Ec_hist,
        x_ref
    )
    ...
    for (z, expected) in anchors
        ...
        @printf(
            "z=%5.1f  quadrature=% .9f  anchor=% .9f  |delta|=%.2e\n",
            z,
            actual,
            expected,
            abs(actual - expected)
    ...
    if payload === nothing
        ...
        for pair_index = 1:n_disorder_pairs
            ...
            h_identity_residual =
                maximum(abs, H_minus + Gamma * H_plus * Gamma) / h_identity_scale
            ...
            pair_residual =
                maximum(abs, m_minus.mu - expected_minus) / maximum(abs, m_plus.mu)
            push!(pair_identity_residuals, pair_residual)
            @printf(
                "  Jx anti-Hermitian rel. residual <= %.3e; pair identity residual %.3e\n",
                max(jres_plus, jres_minus),
                pair_residual
            )
        ...
        if cli.cache_path === nothing
            ...
            println(
                "Wrote matching averaged/individual moment cache: $(abspath(cli.cache_path))",
            )
        end
    else
        ...
        H_plus, Jx_plus, gamma, jres_plus =
            cubic_hamiltonian_and_current(L, disorder; t = t)
        H_minus, Jx_minus, _, jres_minus =
            cubic_hamiltonian_and_current(L, -disorder; t = t)
        build_seconds += time() - build_start
        Gamma = spdiagm(0 => gamma)
        @assert maximum(abs, H_minus + Gamma * H_plus * Gamma) / maximum(abs, H_minus) <
                1e-13
        @assert Jx_plus == Jx_minus
        push!(j_residuals, jres_plus, jres_minus)
        for pair_index = 1:n_disorder_pairs
            ...
            push!(
                pair_identity_residuals,
                maximum(abs, mu_minus - parity_projected_partner(mu_plus)) /
                maximum(abs, mu_plus),
            )
        end
        @printf(
            "Revalidated pair-1 seed=%d and Jx anti-Hermitian residual %.3e.\n",
            seed,
            max(jres_plus, jres_minus)
        )
    end
    ...
    @printf(
        "Maximum antithetic pair identity residual = %.3e\n",
    ...
    sigma_symmetry_residual =
        maximum(abs, sigma_band - reverse(sigma_band)) / maximum(abs, sigma_band)
    @printf(
        "Dense band scan: %d points, max|Sigma|=%.6e, sigma_floor=%.6e\n",
        n_band,
    ...
    for z in (0.0, 4.0)
        ...
        @printf(
            "  z=%4.1f: S(4NC)=% .6f, S(8NC)=% .6f uV/K, rel. diff=%.3e\n",
            z,
            KPM.seebeck_uVK(r4),
            KPM.seebeck_uVK(r8),
            delta
    ...
    for z in z_floor_checks
        ...
        status = if isnan(r_default.S_over_kB_over_e)
            "NaN/warned: default floor masks S"
        else
            "finite: default floor retains S"
        end
        @printf(
            "  z=%5.1f  default=%-34s  override S=% .6f uV/K  L0=%.4e\n",
            z,
            status,
            KPM.seebeck_uVK(r_override),
            r_override.L0
        )
    end
    println(
        "  sigma_min=0.0 values are diagnostics, not reportable insulating-side claims.",
    )

    println("\nDefault-floor equivalence spot checks (same quad_N=4NC):")
    for z in (4.0, -16.0)
        i = findfirst(==(z), z_values)
        guarded = if i === nothing
            KPM.thermoelectric(
            m_avg,
            Ec - z * kT1;
            beta = inv(kT1),
        ...
        default = if haskey(floor_default, z)
            floor_default[z]
        else
            KPM.thermoelectric(
            m_avg,
        ...
        @printf(
            "  z=%5.1f (%s): explicit precomputed floor matches plain default = %s\n",
            z,
            z > 0 ? "conducting" : "insulating",
            string(same_result(guarded, default))
        )
    end
    ...
    for z in scatter_z
        ...
        @printf(
            "Realization scatter at z=%.1f, kBT=%.2f: %s uV/K\n",
            z,
            kT1,
            join((@sprintf("% .3f", value) for value in values), ", ")
        )
    end
    ...
    NC_trunc = 256
    m_trunc = KPM.ConductivityMoments(mu_avg[1:NC_trunc, 1:NC_trunc], a, b, NH, NR)
    n_band_trunc = max(257, 4 * NC_trunc + 1)
    E_band_trunc =
        collect(range(b - band_halfwidth, b + band_halfwidth; length = n_band_trunc))
    sigma_band_trunc = KPM.transport_distribution(m_trunc, E_band_trunc; volume = volume)
    sigma_floor_trunc = 1e-6 * maximum(abs, sigma_band_trunc)
    trunc_changes = Float64[]
    for kBT in kBT_values, i in comparison_indices
        z = z_values[i]
        r_trunc = KPM.thermoelectric(
            m_trunc,
            Ec - z * kBT;
            beta = inv(kBT),
            volume = volume,
    ...
    neg_weights =
        [results[kBT][i].neg_weight for kBT in kBT_values for i in comparison_indices]
    collapse_changes = [
        relative_difference(S_values[kT1][i], S_values[kT2][i]) for i in comparison_indices
    ]
    benchmark_changes = [
        abs(S_values[kBT][i] / S_reference[i] - 1) for kBT in kBT_values for
        i in comparison_indices
    ]
    ...
    @printf(
        "  [%s] neg_weight < 1%%: maximum = %.3f%%\n",
        passfail(max_neg_weight < 0.01),
        100 * max_neg_weight
    )
    @printf(
        "  [%s] NC=%d -> %d maximum S change < 10%%: %.3f%%\n",
        passfail(max_trunc_change < 0.10),
        NC,
        NC_trunc,
        100 * max_trunc_change
    )
    @printf(
        "  [%s] two-temperature maximum collapse difference < 10%%: %.3f%%\n",
        passfail(max_collapse_change < 0.10),
        100 * max_collapse_change
    )
    @printf(
        "  [%s] median |S_KPM/S_ref - 1| < 15%%: %.3f%%\n",
        passfail(median_benchmark_change < 0.15),
        100 * median_benchmark_change
    )
    metallic_S_min =
        minimum(minimum(S_values[kBT][comparison_indices]) for kBT in kBT_values)
    @printf(
        "Metallic-window Seebeck sign check: minimum S = %.6f uV/K (%s).\n",
        metallic_S_min,
        passfail(metallic_S_min > 0)
    )
    ...
    for (i, z) in pairs(z_values)
        @printf(
            "%7.2f  %13.6f  %13.6f  %13.6f  %14.6e  %14.6e\n",
            z,
            S_values[kT1][i],
            S_values[kT2][i],
            S_reference[i],
    ...
    Plots.plot!(
        ...
        linestyle = :dash,
        linewidth = 2,
        label = @sprintf("A(Ec-E)^1.5 fit, %.1f<=E<=%.1f", fit_window...)
    )
    Plots.vline!(
        p1,
        [Ec];
    ...
    p2 = Plots.plot(
        ...
        xlabel = "z = (Ec-mu)/(kBT)",
        ylabel = "S (uV/K)",
        title = "Upper-edge Seebeck",
        xlims = (-4, 8),
        label = "universal x=1.5",
        legend = :topright,
        legend_style...,
    )
    ...
    for (color, kBT) in zip(colors, kBT_values)
        ...
        Plots.plot!(
            ...
            color = color,
            linewidth = 2,
            label = @sprintf("kBT=%.2f", kBT)
        )
    end

    p4 = Plots.plot(;
        xlabel = "z",
        ylabel = "S scatter (uV/K); neg_weight (%)",
        title = "Diagnostics",
        xlims = (-16.8, 8.5),
        ylims = (-60, 105),
        legend = :topright,
    ...
    floor_flags =
        [isfinite(floor_default[z].S_over_kB_over_e) ? 25.0 : -25.0 for z in z_floor_checks]
    Plots.scatter!(
        p4,
        collect(z_floor_checks),
        floor_flags;
        color = :purple,
        marker = :diamond,
        markersize = 6,
        label = "default floor: +25 kept, -25 masked",
    )
    for (z, y) in zip(z_floor_checks, floor_flags)
        Plots.annotate!(
            ...
            y + 8,
            Plots.text(
                @sprintf("override %.0f", KPM.seebeck_uVK(floor_override[z])),
                6,
                :purple,
            ),
        )
    ...
    Plots.annotate!(
        ...
        Plots.text(
            @sprintf(
                "pi*a/NC=%.3f; runtime~%.1fs; PH=%.1e",
                pi * a / NC,
                runtime_after_first_render,
                ph_residual
            ),
            7,
            :left,
