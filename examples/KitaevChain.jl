...
scf = KPM.bdg_solve!(op, [channel]; beta=beta, NC=512, mix=0.3,
                     tol_delta=1e-8, update_density=false, g_rho=1,
                     mixing=:anderson)
amps = [op.D[i, j] for (i, j) in bonds]
proj = KPM.channel_amplitude(channel, amps)
@printf("KPM channel SCF: converged=%s after %d iterations, |Delta_bond| = %.6f\n",
        scf.converged, scf.iterations, abs(proj))
...
for _ in 1:2000
    ...
    i, j = bonds[1]
    Fij = sum(F.vectors[i, s] * conj(F.vectors[j + N, s]) * occ[s] for s in 1:2N)
    Fji = sum(F.vectors[j, s] * conj(F.vectors[i + N, s]) * occ[s] for s in 1:2N)
    Delta_new = -V * (Fij - Fji) / 2
    global Delta_ed = 0.5 * Delta_ed + 0.5 * real(Delta_new)
end
@printf("ED fixed point:  |Delta_bond| = %.6f (relative mismatch %.2e)\n",
        abs(Delta_ed), abs(abs(proj) - abs(Delta_ed)) / abs(Delta_ed))
