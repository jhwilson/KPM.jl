...
"""
function ed_two_energy_response(
    H,
    Jleft,
    Jright;
    beta,
    ...
    for p in eachindex(F.values), q in eachindex(F.values)
        response +=
            Jl[p, q] * Jr[q, p] * (occupations[p] - occupations[q]) /
...
println(
    "  mix=$(mix), NC_scf=$(NC_scf), Np=$(Np_scf), tol_delta=$(tol_delta), tol_n=$(tol_n)",
)
...
for (history_index, entry) in enumerate(result.history)
    if history_index % 10 == 0 || history_index == length(result.history)
        @printf(
            "  %4d  %14.6e  %14.6e  %14.6e  %12.8f  %10.6f\n",
            entry.iter,
            entry.res_delta,
            entry.res_n,
            entry.max_delta,
...
Delta_exact = ComplexF64[
    -U_field[i] * sum(
        F.vectors[i, state] * conj(F.vectors[i+N, state]) * occupations[state] for
        state in eachindex(F.values)
    ) for i = 1:N
]
n_exact = Float64[
    2sum(abs2(F.vectors[i, state]) * occupations[state] for state in eachindex(F.values)) for i = 1:N
]
...
for NC in (64, 256)
    ...
    errors = (
        Delta = maximum(abs.(Delta_kpm .- Delta_exact)),
...
println(
    "  Increasing NC from 64 to 256 changes the update errors by factors " *
...
deterministic = KPM.superfluid_stiffness(
    op,
    pos,
    q;
    psi_in = psi_identity,
...
@printf(
    "  Pi_SC       % .10e%+ .10eim  % .10e%+ .10eim  % .10e%+ .10eim\n",
    real(stochastic.Pi_SC),
    imag(stochastic.Pi_SC),
    real(deterministic.Pi_SC),
    imag(deterministic.Pi_SC),
    real(Pi_SC_ed),
    imag(Pi_SC_ed)
)
@printf(
    "  Pi_N        % .10e%+ .10eim  % .10e%+ .10eim  % .10e%+ .10eim\n",
    real(stochastic.Pi_N),
    imag(stochastic.Pi_N),
    real(deterministic.Pi_N),
    imag(deterministic.Pi_N),
    real(Pi_N_ed),
    imag(Pi_N_ed)
)
@printf(
    "  Ds/pi       % .10e              % .10e              % .10e\n",
    stochastic.Ds_over_pi,
    deterministic.Ds_over_pi,
    Ds_ed
)
println("  deviations from ED:")
@printf(
    "    stochastic:  |Delta Pi_SC|=%.6e, |Delta Pi_N|=%.6e, |Delta Ds/pi|=%.6e\n",
    abs(stochastic.Pi_SC - Pi_SC_ed),
    abs(stochastic.Pi_N - Pi_N_ed),
    abs(stochastic.Ds_over_pi - Ds_ed)
)
@printf(
    "    deterministic: |Delta Pi_SC|=%.6e, |Delta Pi_N|=%.6e, |Delta Ds/pi|=%.6e\n",
    abs(deterministic.Pi_SC - Pi_SC_ed),
    abs(deterministic.Pi_N - Pi_N_ed),
...
function dia_ed(Hdense)
    ...
    return real(
        sum(
            fermi_w(F.values[k]) * dot(view(F.vectors, :, k), Dhat, view(F.vectors, :, k))
...
@printf(
    "  Ds_complete/pi: KPM=% .10e  ED=% .10e  (paramagnetic-only KPM=% .10e)\n",
    deterministic.Ds_over_pi_complete,
    Ds_complete_ed,
    deterministic.Ds_over_pi
)
...
println("\nConventions used")
println(
    "  reduced Nambu block [particle; hole], with hole index i+N; hole_convention=:intervalley (== :singlet for this real h)",
)
println(
    "  Ds/pi is the paramagnetic-only pair; Ds_complete/pi adds the lattice diamagnetic difference",
)
