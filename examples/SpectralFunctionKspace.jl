...
function main()
    ...
    println(
        "Disorder-averaged quasiparticles (N=$N, W=$W, NC=$NC, eta=$ETA, $N_DIS samples):",
    )
    println("      k/pi      E(k)   -2t cos k      Z(k)  gamma(k)")
    for p in eachindex(ks)
        @printf(
            "  %8.4f  %8.4f  %10.4f  %8.4f  %8.4f\n",
            ks[p] / pi,
            Ek[p],
            -2 * t * cos(ks[p]),
            Zk[p],
    ...
    println(
        "max |E(k) - eps_k| = $(round(drift, sigdigits=3)) (weak-disorder shift + grid step)",
    )
    ...
    for p in eachindex(ks)
        ...
        Plots.annotate!(
            ...
            [(
                2.35,
                (p - 1) * offset + 0.12 * offset,
                Plots.text(@sprintf("k=%.2fπ", ks[p] / pi), 7, :right, :black),
            )],
        )
    end
    ...
    Plots.annotate!(
        ...
        [
            (
                Ek[psel] - 0.75,
                0.9 * ymax,
                Plots.text("A(k, E)", 9, :left, Plots.RGB(0 / 255, 114 / 255, 178 / 255)),
            ),
            (
                Ek[psel] + 0.55,
                0.55 * ymax,
                Plots.text("Re G(k, E)", 9, :left, Plots.RGB(213 / 255, 94 / 255, 0 / 255)),
            ),
        ],
    )
        ...
        [
            (
                ...
                Plots.text(
                    ...
                    9,
                    :left,
                    Plots.RGB(0 / 255, 158 / 255, 115 / 255),
                ),
            ),
    ...
    println("\nTiming summary:")
    @printf(
        "  Moments (%d samples x %d momenta):  %8.3f s\n",
        N_DIS,
        length(ks),
        moment_seconds
    )
