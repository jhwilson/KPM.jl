...
function spectral_radius(
    ...
    N = size(op, 1)
    maxiter > 0 ||
        throw(ArgumentError("spectral_radius: maxiter must be positive (got $maxiter)"))
    1 <= miniter <= maxiter || throw(
        ArgumentError(
            "spectral_radius: miniter must satisfy 1 <= miniter <= maxiter (got miniter=$miniter, maxiter=$maxiter)",
        ),
    )
    restarts >= 0 || throw(
        ArgumentError("spectral_radius: restarts must be nonnegative (got $restarts)"),
    )
    ...
    function power_run(v_start)
        ...
        length(v) == N ||
            throw(ArgumentError("spectral_radius: v0 has length $(length(v)); expected $N"))
        nv = norm(v)
...
function Base.getproperty(B::BdGOperator, s::Symbol)
    if s === :Δ
        D = getfield(B, :D)
        D isa Diagonal || throw(
            ArgumentError(
                "BdGOperator: Δ is only defined for onsite (Diagonal) pairing; access op.D",
            ),
        )
        return D.diag
    end
...
function _canonical_hole_convention(s::Symbol; caller::AbstractString = "BdGOperator")
    ...
    throw(
        ArgumentError(
            "$caller: hole_convention must be :intervalley or :conjugate (:singlet is an alias) (got $s)",
        ),
    )
end

function BdGOperator(
    h;
    mu::Real,
    U,
    n = zeros(size(h, 1)),
    Delta = nothing,
    D = nothing,
    hole_convention::Symbol = :intervalley,
    h_hole = nothing,
    assume_intervalley::Bool = false,
)
    N = size(h, 1)
    size(h, 2) == N ||
        throw(ArgumentError("BdGOperator: h must be square (got $(size(h)))"))
    ...
    length(U_vec) == N ||
        throw(ArgumentError("BdGOperator: U has length $(length(U_vec)); expected $N"))
    length(n_vec) == N ||
        throw(ArgumentError("BdGOperator: n has length $(length(n_vec)); expected $N"))
    Delta !== nothing &&
        D !== nothing &&
    ...
    D_stored = if Delta !== nothing
        ...
        length(Δ_vec) == N || throw(
            ArgumentError("BdGOperator: Delta has length $(length(Δ_vec)); expected $N"),
        )
        Diagonal(Δ_vec)
    elseif D !== nothing
        size(D) == (N, N) || throw(
            ArgumentError("BdGOperator: D has size $(size(D)); expected ($N, $N)"),
        )
        D
    else
    ...
    h_hole_stored = if hole_convention === :intervalley
        h_hole === nothing || throw(
            ArgumentError(
                "BdGOperator: h_hole must be nothing for hole_convention=:intervalley",
            ),
        )
        if h_is_matrix && eltype(h) <: Complex
            ...
            if !symmetric && !assume_intervalley
                throw(
                    ArgumentError(
                        "BdGOperator: for complex Hermitian h the reduced same-h hole block is only the correct physics if h is the intervalley-identified operator (h_{-K}^* = h_K); pass assume_intervalley=true to assert that, or use a real-symmetric h.",
                    ),
                )
            end
        end
        ...
        assume_intervalley && throw(
            ArgumentError(
                "BdGOperator: assume_intervalley is meaningless for hole_convention=:conjugate; leave it false",
            ),
        )
        if h_is_matrix &&
           h_hole !== nothing &&
           (
               !(h_hole isa AbstractMatrix) ||
               size(h_hole) != size(h) ||
               !iszero(h_hole - conj(h))
           )
            throw(
                ArgumentError(
                    "BdGOperator: for an assembled h the conjugate hole block is conj(h); a different h_hole would silently invalidate gershgorin_bound and the Hermitian BdG action",
                ),
            )
        end
        if h_hole === nothing
            h_is_matrix || throw(
                ArgumentError(
                    "BdGOperator: matrix-free h with hole_convention=:conjugate requires h_hole, the conjugated normal-state operator",
                ),
            )
            conj(h)
        else
            size(h_hole) == size(h) || throw(
                ArgumentError(
                    "BdGOperator: h_hole has size $(size(h_hole)); expected $(size(h))",
                ),
            )
            h_hole
        end
...
function LinearAlgebra.mul!(
    ...
    N = B.N
    Yp = _nambu_block(Y, N, 1);
    Yh = _nambu_block(Y, N, 2)
    Xp = _nambu_block(X, N, 1);
    Xh = _nambu_block(X, N, 2)
    mul!(Yp, B.h, Xp, α, β)
...
function bdg_assemble(op::BdGOperator)
    _bdg_assemblable(op) || throw(
        ArgumentError(
            "bdg_assemble: requires assembled matrix blocks h, h_hole, and D (matrix-free blocks cannot be assembled)",
        ),
    )
...
function _device_operator(op::BdGOperator, caller::AbstractString)
    ...
    if whichcore() && op_dev === op
        throw(
            ArgumentError(
                "$caller: the active GPU device requires a device-assemblable operator (sparse matrix h and matrix D blocks); matrix-free or dense blocks would mix host and device residences here — sparsify/assemble the blocks or deactivate the GPU",
            ),
        )
    end
    return op_dev
...
struct PairingChannel
    ...
    function PairingChannel(bonds::Vector{Tuple{Int,Int}}, weights, V, parity::Symbol)
        parity in (:even, :odd) || throw(
            ArgumentError("PairingChannel: parity must be :even or :odd (got $parity)"),
        )
        nb = length(bonds)
        weights_vec = if weights isa Number
            fill(ComplexF64(weights), nb)
        else
            collect(ComplexF64, weights)
        end
        V_vec = V isa Number ? fill(Float64(V), nb) : collect(Float64, V)
        length(weights_vec) == nb || throw(
            ArgumentError(
                "PairingChannel: weights has length $(length(weights_vec)); expected $nb",
            ),
        )
        length(V_vec) == nb || throw(
            ArgumentError("PairingChannel: V has length $(length(V_vec)); expected $nb"),
        )
        ...
        for (i, j) in bonds
            i >= 1 && j >= 1 || throw(
                ArgumentError(
                    "PairingChannel: bond indices must be at least 1 (got ($i, $j))",
                ),
            )
            i == j &&
                parity === :odd &&
                throw(
                    ArgumentError(
                        "PairingChannel: onsite bond ($i, $j) is not allowed in an odd channel",
                    ),
                )
            key = minmax(i, j)
            key in seen &&
...
function _validate_channel_ownership(
    channels::AbstractVector{<:PairingChannel},
    ...
    for (channel_index, channel) in pairs(channels)
        for (i, j) in channel.bonds
            ...
            if haskey(owners, key)
                throw(
                    ArgumentError(
                        "$caller: undirected bond $key is shared by channels $(owners[key]) and $channel_index",
                    ),
                )
            end
...
function pairing_matrix(
    ...
    (amplitudes === nothing) != (amplitude === nothing) || throw(
        ArgumentError("pairing_matrix: supply exactly one of amplitudes or amplitude"),
    )
    ...
    amplitudes_vec = if amplitudes === nothing
        ...
        length(amplitudes) == length(channels) || throw(
            ArgumentError(
                "pairing_matrix: amplitudes has length $(length(amplitudes)); expected $(length(channels))",
            ),
        )
        [
            begin
                values = collect(ComplexF64, amplitudes[c])
                length(values) == length(channels[c].bonds) || throw(
                    ArgumentError(
                        "pairing_matrix: amplitudes[$c] has length $(length(values)); expected $(length(channels[c].bonds))",
                    ),
                )
                values
...
"""
function channel_amplitude(channel::PairingChannel, amplitudes::Vector{<:Number})
    length(amplitudes) == length(channel.weights) || throw(
        ArgumentError(
            "channel_amplitude: amplitudes has length $(length(amplitudes)); expected $(length(channel.weights))",
        ),
    )
...
function gershgorin_bound(op::BdGOperator)
    op.h isa AbstractMatrix || throw(
        ArgumentError("gershgorin_bound: BdGOperator must have an assembled matrix h"),
    )
...
function bdg_channel_moments(
    ...
    batch_size > 0 || throw(
        ArgumentError("bdg_channel_moments: batch_size must be positive (got $batch_size)"),
    )
    all(i -> 1 <= i <= N, sites) || throw(
        ArgumentError("bdg_channel_moments: all sites must satisfy 1 <= site <= N=$N"),
    )
    all(b -> 1 <= b[1] <= N && 1 <= b[2] <= N, directed_bonds) || throw(
        ArgumentError(
            "bdg_channel_moments: all directed bond indices must satisfy 1 <= index <= N=$N",
        ),
    )
    ...
    for (bond_index, (i, j)) in pairs(directed_bonds)
        haskey(site_columns, i) || throw(
            ArgumentError(
                "bdg_channel_moments: directed bond ($i, $j) requires source site $i to occur in sites",
            ),
        )
    ...
    for first_site in batch_starts
        ...
        rho_idx_host =
            [(c - 1) * twoN + Int(sites[cg]) for (c, cg) in enumerate(first_site:last_site)]
        bond_idx_host = Int[]
        bond_cols = Int[]
...
function chebyshev_stability_probe(Hs, NH::Integer, NC::Integer; rng = Xoshiro(1))
    NH > 0 ||
        throw(ArgumentError("chebyshev_stability_probe: NH must be positive (got $NH)"))
    size(Hs) == (NH, NH) || throw(
        ArgumentError(
            "chebyshev_stability_probe: Hs has size $(size(Hs)); expected ($NH, $NH)",
        ),
    )
    NC > 0 ||
        throw(ArgumentError("chebyshev_stability_probe: NC must be positive (got $NC)"))
...
where `gamma_c(x) = sum_m kernel(m-1, NC) hn(m-1) mu[m,c] T_{m-1}(x)` and `x_k` are Gauss-Chebyshev nodes. There is no extra
...
"""
function bdg_update(
    mu_rho::AbstractMatrix{<:Real},
    mu_delta::AbstractMatrix{<:Complex},
    a::Real;
    ...
    size(mu_rho) == size(mu_delta) || throw(
        ArgumentError(
            "bdg_update: mu_rho and mu_delta must have the same size (got $(size(mu_rho)) and $(size(mu_delta)))",
        ),
    )
    NC, ns = size(mu_rho)
    length(U) == ns ||
        throw(ArgumentError("bdg_update: U has length $(length(U)); expected $ns"))
...
"""
function bdg_channel_update(
    mu_rho::AbstractMatrix{<:Real},
    mu_F::AbstractMatrix{<:Complex},
    a::Real;
    channels::Vector{PairingChannel},
    directed_bonds::Vector{Tuple{Int,Int}},
    beta::Real,
    ...
    size(mu_rho, 1) == size(mu_F, 1) || throw(
        ArgumentError(
            "bdg_channel_update: mu_rho and mu_F must have the same number of rows (got $(size(mu_rho, 1)) and $(size(mu_F, 1)))",
        ),
    )
    size(mu_F, 2) == length(directed_bonds) || throw(
        ArgumentError(
            "bdg_channel_update: mu_F has $(size(mu_F, 2)) columns; expected $(length(directed_bonds)) directed bonds",
        ),
    )
    beta > 0 ||
        throw(ArgumentError("bdg_channel_update: beta must be positive (got $beta)"))
    ...
    for c in eachindex(channels)
        ...
        for b in eachindex(channel.bonds)
            ...
            Fsym = if i == j
                ...
                (F[directed_index[(i, j)]] + parity_sign * F[directed_index[(j, i)]]) / 2
            end
...
Base.show(io::IO, r::BdGSCFResult) = print(
    io,
...
function _channel_structure(
    op::BdGOperator,
    channels::Vector{PairingChannel},
    caller::AbstractString,
)
    op.D isa SparseMatrixCSC || throw(
        ArgumentError(
            "$caller: channel self-consistency requires op.D isa SparseMatrixCSC",
        ),
    )
    ...
    for c in eachindex(channels)
        ...
        for b in eachindex(channel.bonds)
            ...
            if i == j
                ...
                haskey(nz_indices, (j, i)) || throw(
                    ArgumentError("$caller: op.D is missing structural entry ($j, $i)"),
                )
...
function _read_channel_amplitudes(
    op::BdGOperator,
    structure::Vector{Vector{Tuple{Int,Int}}},
)
    values = nonzeros(op.D)
    return [
        [ComplexF64(values[structure[c][b][1]]) for b in eachindex(structure[c])] for
        c in eachindex(structure)
    ]
end

function _write_channel_amplitudes!(
    op::BdGOperator,
    channels::Vector{PairingChannel},
    structure::Vector{Vector{Tuple{Int,Int}}},
    amplitudes,
    ...
    length(amplitudes) == length(channels) || throw(
        ArgumentError("bdg_restore!: checkpoint amplitudes has invalid channel count"),
    )
    values = nonzeros(op.D)
    for c in eachindex(channels)
        length(amplitudes[c]) == length(channels[c].bonds) || throw(
            ArgumentError("bdg_restore!: checkpoint amplitudes[$c] has invalid length"),
        )
...
function _checkpoint_channel_data(channels::Vector{PairingChannel})
    return [
        (
            bonds = copy(channel.bonds),
            weights = copy(channel.weights),
            V = copy(channel.V),
...
function bdg_restore!(op::BdGOperator, path::AbstractString)
    ...
    state.version == 3 && throw(
        ArgumentError(
            "bdg_restore!: version-3 checkpoint contains pairing channels; call bdg_restore!(op, channels, path)",
        ),
    )
    state.version in (1, 2) || throw(
        ArgumentError("bdg_restore!: unsupported checkpoint version $(state.version)"),
    )
    state.N == op.N || throw(
        ArgumentError("bdg_restore!: checkpoint has N=$(state.N); expected N=$(op.N)"),
    )
    state.U ≈ op.U || throw(
        ArgumentError("bdg_restore!: checkpoint interaction U does not match the operator"),
    )
    length(state.delta) == op.N ||
        throw(ArgumentError("bdg_restore!: checkpoint delta has invalid length"))
    length(state.n) == op.N ||
        throw(ArgumentError("bdg_restore!: checkpoint n has invalid length"))
    op.Δ .= state.delta
    op.n .= state.n
    op.μ = state.mu
    params = state.version == 1 ? nothing : state.params
    return copy(state.history),
    state.v_power === nothing ? nothing : copy(state.v_power),
    params
end
...
function bdg_restore!(
    ...
    state.version in (1, 2) && throw(
        ArgumentError(
            "bdg_restore!: channel restore requires a version-3 checkpoint; got legacy version $(state.version)",
        ),
    )
    state.version == 3 || throw(
        ArgumentError("bdg_restore!: unsupported checkpoint version $(state.version)"),
    ...
    state.hole_convention === op.hole_convention || throw(
        ArgumentError(
            "bdg_restore!: checkpoint hole convention does not match the operator",
        ),
    )
    length(state.n) == op.N ||
        throw(ArgumentError("bdg_restore!: checkpoint n has invalid length"))
    ...
    for c in eachindex(channels)
        ...
        saved.V ≈ current.V || throw(
            ArgumentError("bdg_restore!: checkpoint channel $c coupling V does not match"),
        )
        saved.parity === current.parity || throw(
            ArgumentError("bdg_restore!: checkpoint channel $c parity does not match"),
        )
    end
    ...
    for (i, j) in sort!(collect(fixed_slots))
        if !((i, j) in channel_slots) && !isequal(op.D[i, j], saved_D[i, j])
            throw(
                ArgumentError(
                    "bdg_restore!: pairing entry outside declared channels at ($i, $j) does not match the checkpoint",
                ),
            )
        end
    end
...
function _bdg_scf_driver!(
    ...
    mixing::Symbol,
    anderson_history::Integer,
    anderson_delay::Integer,
    anderson_max_step::Real,
    verbose::Integer,
)
    target_filling !== nothing &&
        !update_density &&
        throw(
            ArgumentError(
                "bdg_solve!: target_filling cannot be combined with update_density=false: the filling estimator requires density updates",
            ),
        )
    ...
    function inner_solve!()
        ...
        for local_iter = 1:Int(maxiter)
            ...
            rad > 0 ||
                throw(ArgumentError("bdg_solve!: BdG operator has zero spectral radius"))
            ...
            n_pass = update_density ? (res_n_abs <= tol_n || res_n_rel <= tol_n) : true
            ...
            if mixing === :anderson
                ...
                if p >= 2 && local_iter > anderson_delay
                    dX = reduce(hcat, (X_hist[j+1] .- X_hist[j] for j = 1:(p-1)))
                    dF = reduce(hcat, (F_hist[j+1] .- F_hist[j] for j = 1:(p-1)))
                    gamma = try
                        dF \ F_k
                    catch
                        nothing
                    ...
                    if gamma !== nothing && all(isfinite, gamma)
                        ...
                        if norm(candidate .- X_k) <=
                           anderson_max_step * max(damped_step, eps())
                            X_next = candidate
                        end
                    end
                end
            ...
            entry = (
                iter = iter,
                res_delta = Float64(res_d_abs),
                res_n = Float64(res_n_abs),
                max_delta = Float64(max_delta()),
                mean_n = Float64(sum(op.n) / op.N),
                mu = op.μ,
                a = Float64(a),
            )
            push!(history, entry)
            ...
            verbose >= 1 && println(
                "BdG iter $(iter): res_delta=$(res_d_abs), res_n=$(res_n_abs), mu=$(op.μ), a=$(a)",
            )
            converged && break
        end
    ...
    isfinite(mu_lo) && isfinite(mu_hi) || throw(
        ArgumentError("bdg_solve!: target_filling requires finite mu_bracket endpoints"),
    )
    ...
    function filling_error!(mu)
        ...
        result.converged || error(
            "bdg_solve!: target-filling inner solve did not converge at mu=$(op.μ); raise maxiter before continuing the bisection",
        )
    ...
    error(
        "bdg_solve!: target-filling bisection did not converge after $(mu_maxiter) iterations; raise mu_maxiter or mu_tol",
    )
end
...
function bdg_solve!(
    ...
    anderson_history::Integer = 6,
    anderson_delay::Integer = 5,
    anderson_max_step::Real = 50.0,
    verbose::Integer = 0,
)
    op.D isa Diagonal || throw(
        ArgumentError(
            "bdg_solve!: the onsite gap equation requires Diagonal (onsite) pairing; bond-channel self-consistency is not yet implemented",
        ),
    )
    ...
    0 < mix <= 1 ||
        throw(ArgumentError("bdg_solve!: mix must satisfy 0 < mix <= 1 (got $mix)"))
    tol_delta >= 0 ||
        throw(ArgumentError("bdg_solve!: tol_delta must be nonnegative (got $tol_delta)"))
    tol_n >= 0 || throw(ArgumentError("bdg_solve!: tol_n must be nonnegative (got $tol_n)"))
    maxiter > 0 ||
        throw(ArgumentError("bdg_solve!: maxiter must be positive (got $maxiter)"))
    batch_size > 0 ||
        throw(ArgumentError("bdg_solve!: batch_size must be positive (got $batch_size)"))
    0 < rescale_eps < 2 || throw(
        ArgumentError(
            "bdg_solve!: rescale_eps must satisfy 0 < rescale_eps < 2 (got $rescale_eps)",
        ),
    )
    mu_tol >= 0 ||
        throw(ArgumentError("bdg_solve!: mu_tol must be nonnegative (got $mu_tol)"))
    mu_maxiter > 0 ||
        throw(ArgumentError("bdg_solve!: mu_maxiter must be positive (got $mu_maxiter)"))
    checkpoint_every > 0 || throw(
        ArgumentError(
            "bdg_solve!: checkpoint_every must be positive (got $checkpoint_every)",
        ),
    )
    restart !== nothing &&
        target_filling !== nothing &&
        throw(
            ArgumentError(
                "bdg_solve!: restart cannot be combined with target_filling because the bisection state is not checkpointed",
            ),
        )
    mixing in (:linear, :anderson) || throw(
        ArgumentError("bdg_solve!: mixing must be :linear or :anderson (got $mixing)"),
    )
    anderson_history >= 1 || throw(
        ArgumentError(
            "bdg_solve!: anderson_history must be positive (got $anderson_history)",
        ),
    )
    anderson_delay >= 0 || throw(
        ArgumentError(
            "bdg_solve!: anderson_delay must be nonnegative (got $anderson_delay)",
        ),
    )
    anderson_max_step > 0 || throw(
        ArgumentError(
            "bdg_solve!: anderson_max_step must be positive (got $anderson_max_step)",
        ),
    )
    ...
    history, v_power, saved_params =
        restart === nothing ? (NamedTuple[], nothing, nothing) : bdg_restore!(op, restart)
    if saved_params !== nothing
        differences = String[]
        for name in propertynames(params)
            # older checkpoints may predate newly added parameters
            saved =
                hasproperty(saved_params, name) ? getproperty(saved_params, name) : missing
            current = getproperty(params, name)
            isequal(saved, current) || push!(
                differences,
                "$(name): saved=$(repr(saved)), current=$(repr(current))",
            )
        end
    ...
    Nsites = op.N
    pack_state() = if update_density
        vcat(real.(op.Δ), imag.(op.Δ), copy(op.n))
    else
        vcat(real.(op.Δ), imag.(op.Δ))
    end
    pack_out(update) = if update_density
    ...
    function compute_update(Hs, a)
        mu_rho, mu_delta =
            bdg_site_moments(Hs, op.N, 1:op.N, Int(NC); batch_size = Int(batch_size))
        return bdg_update(
            mu_rho,
            mu_delta,
            a;
...
function bdg_solve!(
    ...
    op.hole_convention === :conjugate || throw(
        ArgumentError(
            "bdg_solve!: pairing-channel self-consistency requires hole_convention=:conjugate because transpose-parity symmetrization presumes same-block fermionic exchange; an intervalley partner-valley pairing map is not implemented",
        ),
    )
    beta > 0 || throw(ArgumentError("bdg_solve!: beta must be positive (got $beta)"))
    NC >= 2 || throw(ArgumentError("bdg_solve!: NC must be at least 2 (got $NC)"))
    ...
    history, v_power, saved_params = if restart === nothing
        (NamedTuple[], nothing, nothing)
    else
        bdg_restore!(op, channels, restart)
    end
    if saved_params !== nothing
        differences = String[]
        for name in propertynames(params)
    ...
    function set_state!(x)
        flat = @views complex.(x[1:namplitudes], x[(namplitudes+1):2namplitudes])
        offset = 0
        ...
        update_density && (@views op.n .= x[(2namplitudes+1):(2namplitudes+op.N)])
        return nothing
    end
    function compute_update(Hs, a)
    ...
    function linear_update!(update)
        ...
        for c in eachindex(amplitudes), b in eachindex(amplitudes[c])
            amplitudes[c][b] = (1 - mix) * amplitudes[c][b] + mix * amplitudes_new[c][b]
        end
        _write_channel_amplitudes!(op, channels, structure, amplitudes)
        if update_density
            @. op.n = (1 - mix) * op.n + mix * n_new
    ...
    checkpoint!(history, v_power, params) =
        bdg_checkpoint(checkpoint_path, op, channels, amplitudes, history, v_power, params)
