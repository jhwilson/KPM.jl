...
function chebyshev_iter_single(
    H,
...
function chebyshev_iter_single(H, V_pp_in::ASA, V_p_in::ASA)
    ...
    if Threads.nthreads() == 1 ||
       length(V_pp_in) * length(first(V_pp_in)) <= _CHEB_SERIAL_CUTOFF
...
function _check_chebyshev_columns(slot::AbstractMatrix, iteration::Integer)
    ...
    if !(max_norm <= 1.5)
        error(
            "Chebyshev recurrence is unstable at iteration $iteration (maximum column norm $max_norm > 1.5); use rescale(...; bound=:gershgorin) or a larger eps.",
        )
    end
    return nothing
end
...
function _check_chebyshev_columns(
    slot::AbstractMatrix,
    iteration::Integer,
    ...
    if !(growth_sq <= 1.5^2)
        error(
            "Chebyshev recurrence is unstable at iteration $iteration (maximum column growth $(sqrt(growth_sq)) > 1.5); use a larger eps (rescaling safety margin) in rescale/normalizeH, or bound=:gershgorin for BdG operators.",
        )
    end
    return nothing
end
