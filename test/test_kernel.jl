using KPM:
    hn, JacksonKernel, LorentzKernels, muND_apply_kernel_and_h, mu2D_apply_kernel_and_h
...
for kernel in [
    ...
    mu_1d_tilde =
        KPM.maybe_to_host(KPM.muND_apply_kernel_and_h(mu_1d, NC, kernel; dims = [1]))
    ...
    mu_3d_tilde = KPM.maybe_to_host(
        KPM.muND_apply_kernel_and_h(mu_3d, NC, KPM.JacksonKernel; dims = [1, 2, 3]),
    )
    ...
    @test all(
        mu_3d_tilde[1, :, :]/mu_3d_tilde[1, 1, 1] .≈
        mu_3d_tilde[2, :, :]/mu_3d_tilde[2, 1, 1],
    )
end
