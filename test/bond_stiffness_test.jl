...
function bond_stiffness_response(D)
    op = KPM.BdGOperator(
        BOND_STIFFNESS_H; mu=BOND_STIFFNESS_MU,
        U=BOND_STIFFNESS_INTERACTION, n=BOND_STIFFNESS_DENSITY, D=D,
        hole_convention=:conjugate)
    return KPM.superfluid_stiffness(
        op, BOND_STIFFNESS_POS, BOND_STIFFNESS_QY;
        beta=BOND_STIFFNESS_BETA, eta=0.0, NC=256,
        psi_in=BOND_STIFFNESS_PSI, volume=BOND_STIFFNESS_VOLUME,
        g_J=BOND_STIFFNESS_G_J, disp=BOND_STIFFNESS_DISP,
...
@testset "zero-gap cancellation with structural bond zeros" begin
    D0 = KPM.pairing_matrix(
        BOND_STIFFNESS_N, [BOND_STIFFNESS_CHANNEL]; amplitude=0.0)
...
@testset "rigid d-wave stiffness vs free-energy curvature" begin
    D = KPM.pairing_matrix(
        BOND_STIFFNESS_N, [BOND_STIFFNESS_CHANNEL]; amplitude=0.45)
    D_SC = Matrix(D)
    ...
    free_energy(D_or_zero, A) = ed_bdg_free_energy(
        BOND_STIFFNESS_H, BOND_STIFFNESS_POS, BOND_STIFFNESS_DISP,
        BOND_STIFFNESS_MU, BOND_STIFFNESS_INTERACTION, BOND_STIFFNESS_DENSITY,
        Matrix(D_or_zero), A; q=BOND_STIFFNESS_QY, dir=1,
        beta=BOND_STIFFNESS_BETA)
    curvature(D_or_zero, delta) =
        (free_energy(D_or_zero, delta) - 2free_energy(D_or_zero, 0.0) +
         free_energy(D_or_zero, -delta)) / delta^2
    ...
    anchor_halfstep = (2BOND_STIFFNESS_G_J / BOND_STIFFNESS_VOLUME) *
    ...
    println("bond free-energy curvatures delta=1e-3: SC=$(Fpp_SC), N=$(Fpp_N), anchor=$(anchor)")
    println("bond free-energy curvatures delta=5e-4: SC=$(Fpp_SC_halfstep), N=$(Fpp_N_halfstep), anchor=$(anchor_halfstep)")
    println("bond complete stiffness: curvature anchor=$(anchor), KPM=$(r.Ds_over_pi_complete), relerr=$(stiffness_relerr)")
    println("bond curvature step relative error=$(curvature_relerr)")
    println("bond paramagnetic-only stiffness=$(r.Ds_over_pi), diamagnetic correction=$(r.Dia_SC - r.Dia_N)")
...
@testset "paramagnetic response vs dense ED" begin
    D = KPM.pairing_matrix(
        BOND_STIFFNESS_N, [BOND_STIFFNESS_CHANNEL]; amplitude=0.45)
    r = bond_stiffness_response(D)
    xi = Matrix{ComplexF64}(BOND_STIFFNESS_H) - BOND_STIFFNESS_MU * I -
         Diagonal(BOND_STIFFNESS_INTERACTION .* BOND_STIFFNESS_DENSITY ./ 2)
    H = [xi Matrix{ComplexF64}(D);
         adjoint(Matrix{ComplexF64}(D)) -conj(xi)]
    Jq = KPM.nambu_current_q(
        BOND_STIFFNESS_H, BOND_STIFFNESS_POS, BOND_STIFFNESS_QY;
        dir=1, disp=BOND_STIFFNESS_DISP, hole_convention=:conjugate)
