...
@testset "identical position data => identical response; different embeddings => different" begin
    ...
    stiffness(pos, disp) = KPM.superfluid_stiffness(
        ...
        NC = 256,
        NR = 6,
        rng = KPM.Random.Xoshiro(7),
        volume = SSH_L,
        include_diamagnetic = true,
    )
    r1 =
        @test_logs (:warn, r"longitudinal") match_mode=:any stiffness(SSH_POS_A, SSH_DISP_A)
    r1b =
        @test_logs (:warn, r"longitudinal") match_mode=:any stiffness(SSH_POS_A, SSH_DISP_A)
    r2 =
        @test_logs (:warn, r"longitudinal") match_mode=:any stiffness(SSH_POS_B, SSH_DISP_B)
    println(
        "SSH stiffness: split=$(r1.Ds_over_pi_complete), colocated=$(r2.Ds_over_pi_complete), Dia_SC=($(r1.Dia_SC), $(r2.Dia_SC))",
    )
