...
@testset "two-site analytic pairing-channel fixed point" begin
    ...
    beta = 8.0
    V = 3.0
    equation(R) = V / (4R) * (tanh(beta * (R + t) / 2) + tanh(beta * (R - t) / 2)) - 1
    ...
    F12 = sum(
        eig.vectors[1, s] * conj(eig.vectors[4, s]) * occ[s] for s in eachindex(eig.values)
    )
    F_closed = -delta_exact / (4R) * (tanh(beta * (R + t) / 2) + tanh(beta * (R - t) / 2))
...
@testset "grand-potential stationarity (independent variational gate)" begin
    function Omega(h, mu, beta, amps, channel)
        ...
        interaction = sum(abs2(amps[b]) / channel.V[b] for b in eachindex(amps))
        quasiparticles = sum(log(2cosh(beta * E / 2)) for E in eigvals(Hermitian(H)))
    ...
    for _ = 1:10_000
        ...
        eig = eigen(Hermitian(channel_dense(h_ring, mu_ring, zeros(N), zeros(N), D)))
        occ = KPM.fermiFunctions(0.0, beta).(eig.values)
...
@testset "bond moments vs exact eigenvector sums" begin
    ...
    D = KPM.pairing_matrix(N, [channel]; amplitudes = [fill(delta_p, length(bonds))])
    ...
    for NC in (64, 256)
        _, mu_F =
            KPM.bdg_channel_moments(rh.H, N, collect(1:N), directed, NC; batch_size = 3)
...
@testset "Kitaev channel SCF vs ED and U(1) covariance" begin
    ...
    phase_errors =
        angle.(exp.(im .* (angle.(phase_amplitudes) .- angle.(amplitudes) .- 0.7)))
...
@testset "open-chain Majorana check" begin
    ...
    for mu in (0.5, 3.0)
        values = sort(abs.(eigvals(Hermitian(channel_dense(h, mu, zeros(N), zeros(N), D)))))
        if mu == 0.5
            @test values[1] < 1e-3
            @test values[2] < 1e-3
            @test values[3] > 0.1
...
@testset "legacy onsite equivalence" begin
    ...
    channel = KPM.PairingChannel([(i, i) for i = 1:N], 1.0, U, :even)
    op_legacy = KPM.BdGOperator(h; mu = -0.3, U = U, n = n0, Delta = delta0)
    D = KPM.pairing_matrix(N, [channel]; amplitudes = [delta0])
    op_channel =
        KPM.BdGOperator(h; mu = -0.3, U = U, n = n0, D = D, hole_convention = :conjugate)
    ...
    channel_delta = ComplexF64[op_channel.D[i, i] for i = 1:N]
    ...
    local_rho, local_F = KPM.bdg_site_moments(rh.H, N, sites, 64; batch_size = 3)
    channel_rho, channel_F =
        KPM.bdg_channel_moments(rh.H, N, sites, onsite, 64; batch_size = 3)
...
@testset "pairing-channel checkpoint v3" begin
    ...
    mktempdir() do dir
        ...
        # update_density=false: n is never touched by the solver on any
        # device, so it must be bitwise identical even under GPU.
        ...
        legacy =
            KPM.BdGOperator(h; mu = 0.2, U = 0.0, n = zeros(N), Delta = fill(0.3 + 0im, N))
        @test_throws ArgumentError KPM.bdg_restore!(legacy, checkpoint)
        legacy_checkpoint = joinpath(dir, "legacy-checkpoint.bin")
        KPM.bdg_checkpoint(legacy_checkpoint, legacy, NamedTuple[], nothing, (beta = 8.0,))
        @test_throws ArgumentError KPM.bdg_restore!(make_op(), [channel], legacy_checkpoint)
    end
end
...
@testset "pairing-channel API validation" begin
    ...
    reversed = KPM.PairingChannel(reversed_bonds, [-1.0, 1.0, 1.0], 3.0, :odd)
    oriented_amplitudes = ComplexF64[0.3, 0.4im, -0.2+0.1im]
    reversed_amplitudes = copy(oriented_amplitudes)
    reversed_amplitudes[1] *= -1
    D_oriented = KPM.pairing_matrix(3, [oriented]; amplitudes = [oriented_amplitudes])
    D_reversed = KPM.pairing_matrix(3, [reversed]; amplitudes = [reversed_amplitudes])
    ...
    overlap_op =
        KPM.BdGOperator(h; mu = 0.0, U = 0.0, D = overlap_D, hole_convention = :conjugate)
    @test_throws ArgumentError KPM.bdg_solve!(overlap_op, [c1, c2]; beta = 1.0)
    intervalley_op =
        KPM.BdGOperator(h; mu = 0.0, U = 0.0, D = overlap_D, hole_convention = :intervalley)
    @test_throws ArgumentError KPM.bdg_solve!(intervalley_op, [c1]; beta = 1.0)

    missing_D = sparse([1], [2], ComplexF64[0.2], 2, 2)
    missing_op =
        KPM.BdGOperator(h; mu = 0.0, U = 0.0, D = missing_D, hole_convention = :conjugate)
    ...
    reversed_ring_channel =
        KPM.PairingChannel(reversed_ring_bonds, reversed_ring_weights, 3.0, :odd)
    ...
    @test occursin(
        "target-filling bisection did not converge",
