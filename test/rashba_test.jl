...
function ed_fixed_point(h, channel; mu, beta, seed=0.2, mix=0.5,
...
const RASHBA_CHANNEL = KPM.PairingChannel([(1, 2), (3, 4)], 1.0, RASHBA_V,
                                           :odd)
...
@testset "singlet is odd with explicit spin" begin
    ...
    tauy = [zeros(ComplexF64, 4, 4) -im * I4;
...
@testset "channel update vs ED" begin
    ...
    op = KPM.BdGOperator(RASHBA_H; mu=RASHBA_MU, U=0.0, n=zeros(4), D=D0,
                         hole_convention=:conjugate)
    rh = KPM.rescale(op)
    errors = Float64[]
    for NC in (128, 512)
        moments = KPM.bdg_channel_moments(rh, [RASHBA_CHANNEL]; NC=NC,
                                           g_rho=1)
...
@testset "SCF vs ED fixed point" begin
    delta_ed, residual_ed = ed_fixed_point(RASHBA_H, RASHBA_CHANNEL;
                                            mu=RASHBA_MU, beta=RASHBA_BETA)
    @test residual_ed < 1e-11

    D0 = KPM.pairing_matrix(4, [RASHBA_CHANNEL]; amplitude=0.2)
    op = KPM.BdGOperator(RASHBA_H; mu=RASHBA_MU, U=0.0, n=zeros(4), D=D0,
                         hole_convention=:conjugate)
    result = KPM.bdg_solve!(op, [RASHBA_CHANNEL]; beta=RASHBA_BETA, NC=512,
                             g_rho=1, mix=0.3, tol_delta=1e-8,
...
@testset "lambda=0 reduces to the legacy reduced-singlet solver" begin
    ...
    D0 = KPM.pairing_matrix(4, [RASHBA_CHANNEL]; amplitude=0.2)
    op_explicit = KPM.BdGOperator(h0; mu=RASHBA_MU, U=0.0, n=zeros(4), D=D0,
                                  hole_convention=:conjugate)
    result_explicit = KPM.bdg_solve!(
        op_explicit, [RASHBA_CHANNEL]; beta=RASHBA_BETA, NC=512, g_rho=1,
        mix=0.3, tol_delta=1e-8, update_density=false)
    delta_explicit = abs.(ComplexF64[op_explicit.D[i, j]
                                      for (i, j) in RASHBA_CHANNEL.bonds])

    h2 = sparse([0.0 -RASHBA_T; -RASHBA_T 0.0])
    op_legacy = KPM.BdGOperator(h2; mu=RASHBA_MU, U=RASHBA_V, n=zeros(2),
                                Delta=fill(0.2, 2),
                                hole_convention=:conjugate)
    result_legacy = KPM.bdg_solve!(op_legacy; beta=RASHBA_BETA, NC=512,
                                   mix=0.3, tol_delta=1e-8,
                                   update_density=false)
