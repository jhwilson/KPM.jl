...
@testset "BdG matrix-free action vs assembled" begin
    ...
    op_free = KPM.BdGOperator(
        KPM.ScaledOperator(BdG_h, 1.0, 0.0);
        mu = BdG_mu,
        U = BdG_U,
        n = BdG_n,
        Delta = BdG_Delta,
    ...
    op_dense =
        KPM.BdGOperator(Matrix(BdG_h); mu = BdG_mu, U = BdG_U, n = BdG_n, Delta = BdG_Delta)
    @test Matrix(KPM.bdg_assemble(op_dense)) ≈ action_matrix(op_dense) atol=1e-12
    op_herm = KPM.BdGOperator(
        Hermitian(sparse(BdG_h));
        mu = BdG_mu,
        U = BdG_U,
        n = BdG_n,
        Delta = BdG_Delta,
...
@testset "self-consistency: fixed point, seeds, U=0, phase, restart" begin
    ...
    for _ = 1:10_000
        ...
        Delta_new = ComplexF64[
            -U[i] * sum(
                F.vectors[i, j] * conj(F.vectors[i+N, j]) * occupations[j] for
                j in eachindex(F.values)
            ) for i = 1:N
        ]
        n_new = Float64[
            2sum(abs2(F.vectors[i, j]) * occupations[j] for j in eachindex(F.values))
            for i = 1:N
        ]
    ...
    op_seed =
        KPM.BdGOperator(h; mu = mu, U = U, n = n_initial, Delta = fill(0.4 - 0.2im, N))
    res_seed = KPM.bdg_solve!(
        op_seed;
        beta = beta,
        NC = 512,
        mix = 0.15,
    ...
    op_u0 =
        KPM.BdGOperator(h; mu = mu, U = 0.0, n = n_initial, Delta = fill(0.3 + 0.4im, N))
    res_u0 = KPM.bdg_solve!(
        op_u0;
        beta = beta,
        NC = 512,
        mix = 0.3,
    ...
    mktempdir() do dir
        ...
        op_bad_u =
            KPM.BdGOperator(h; mu = mu, U = U .+ 1, n = n_initial, Delta = Delta_initial)
        ...
        op_warn = KPM.BdGOperator(h; mu = mu, U = U, n = n_initial, Delta = Delta_initial)
...
@testset "ScaledOperator and spectral_radius bounds" begin
    ...
    println(
        "isolated-edge scaling: power a=$(rh_edge.a), Gershgorin a=$(rh_edge_certified.a), required=$(required_scale)",
    )
...
@testset "one-site analytic update (Eqs 26-27)" begin
    ...
    println(
        "one-site errors: Delta NC=128 $(err128_Delta), NC=512 $(err512_Delta); n NC=128 $(err128_n), NC=512 $(err512_n)",
    )
...
@testset "ring update vs exact eigenvector sums" begin
    ...
    n_exact = Float64[
        2sum(abs2(F.vectors[i, j]) * occupations[j] for j in eachindex(F.values)) for
        i = 1:N
    ]
    ...
    println(
        "ring errors: Delta NC=64 $(err64_Delta), NC=256 $(err256_Delta); n NC=64 $(err64_n), NC=256 $(err256_n)",
    )
    ...
    mu_rho, mu_delta =
        KPM.bdg_site_moments(rh.H, op.N, collect(1:op.N), 256; batch_size = 3)
    n_raw, Delta_raw = KPM.bdg_update(mu_rho, mu_delta, rh.a; U = op.U, beta = beta)
    @test n_raw == n256
...
@testset "singlet hole convention (complex flux ring)" begin
    ...
    U = fill(2.0, N)
    n = fill(0.9, N)
    Delta = [0.4exp(0.3im), 0.4exp(1.1im), 0.4exp(-0.7im), 0.4exp(2.0im)]
    ...
    alpha = 1.2 - 0.3im
    beta_mul = -0.4 + 0.7im
    @test mul!(copy(y), op_s, x, alpha, beta_mul) ≈ alpha * Hs * x + beta_mul * y atol=1e-12
    ...
    println(
        "flux-ring PH mismatch: singlet=$(maximum(abs.(ev .+ reverse(ev)))), intervalley=$(intervalley_pairing_mismatch)",
    )
    ...
    update_delta_relerr =
        maximum(abs.(Delta_kpm .- Delta_exact)) / maximum(abs, Delta_exact)
    update_n_relerr = maximum(abs.(n_kpm .- n_exact)) / maximum(abs, n_exact)
    println(
        "flux-ring singlet update: Delta relerr=$(update_delta_relerr), n relerr=$(update_n_relerr)",
    )
    ...
    for _ = 1:10_000
        ...
        Delta_new = ComplexF64[
            -U_scf[i] * sum(
                F_ed.vectors[i, j] * conj(F_ed.vectors[i+N, j]) * occupations_ed[j] for
                j in eachindex(F_ed.values)
            ) for i = 1:N
        ]
    ...
    scf_delta_relerr =
        maximum(abs.(abs.(op_scf.Δ) .- abs.(Delta_ed))) / maximum(abs, Delta_ed)
    println(
        "flux-ring singlet SCF: |Delta_ed|=$(abs(Delta_ed[1])), Delta relerr=$(scf_delta_relerr)",
    )
...
@testset "Anderson mixing: same fixed point, fewer iterations" begin
    ...
    res_and = KPM.bdg_solve!(
        ...
        anderson_history = 6,
        anderson_delay = 10,
        anderson_max_step = 20.0,
        tol_delta = 1e-9,
        tol_n = 1e-9,
        maxiter = 400,
    )
    println(
        "mixing iterations: linear=$(res_lin.iterations), anderson=$(res_and.iterations)",
    )
