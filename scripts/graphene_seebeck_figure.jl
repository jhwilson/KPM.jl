...
try
    ...
    println(stderr, "QuadGK.jl is required (same example-only dependency as GrapheneSeebeck.jl).")
    rethrow(err)
end
...
expected_params() = (; schema=1, Nx, Ny, t, a, b, NC, NR, arr_size, seed,
...
function seebeck_reference(eta::Real)
    ...
    A0, _ = quadgk(y -> abs(y) * fermi_weight(y - eta_f),
                   lower, 0.0, upper; rtol=2e-13, atol=1e-14)
    A1, _ = quadgk(y -> (y - eta_f) * abs(y) * fermi_weight(y - eta_f),
                   lower, 0.0, upper; rtol=2e-13, atol=1e-14)
...
function main()
    ...
    for kBT in kBT_values
        S[kBT] = [KPM.seebeck_uVK(
                      KPM.thermoelectric(mxx, eta * kBT;
                                         beta=inv(kBT), volume=area, g_J=g_J,
                                         sigma_min=0.0, quad_N=quad_N))
                  for eta in eta_values]
    end
    accepted = Dict(kBT => [abs(eta * kBT) <= 0.35t + 10eps(Float64)
                            for eta in eta_values]
                    for kBT in kBT_values)
    ...
    open(joinpath(cli.outdir, "seebeck.csv"), "w") do io
        ...
        writedlm(io, hcat(eta_values, seebeck_reference.(eta_values),
                          S[kT1], Int.(accepted[kT1]),
                          S[kT2], Int.(accepted[kT2])), ',')
    end
    ...
    println("Wrote sigma.csv, seebeck.csv, reference_dense.csv, provenance.txt to $(abspath(cli.outdir))")
end
