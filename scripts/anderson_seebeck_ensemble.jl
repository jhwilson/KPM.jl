...
function main()
    ...
    @printf(
        "task %d/%d: L=%d NH=%d NC=%d NR=%d pair=%d seed=%d\n",
        cli.task,
        n_tasks,
        L,
        NH,
    ...
    @assert maximum(abs, H_minus + Gamma * H_plus * Gamma) / maximum(abs, H_minus) < 1e-13
    ...
    t_plus = @elapsed m_plus = KPM.cond_moments(
        h_plus,
        Jx,
        Jx;
        NC = NC,
    ...
    t_minus = @elapsed m_minus = KPM.cond_moments(
        h_minus,
        Jx,
        Jx;
        NC = NC,
